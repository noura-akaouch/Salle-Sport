...
public class MembreDAO {
    ...
    public void ajouter(Membre m) {
        // ✅ etat ajouté
        String sql = "INSERT INTO membre (nom, prenom, telephone, email, date_inscription, etat) " +
                "VALUES (?,?,?,?,?,?)";
        ...
        try (PreparedStatement ps = Database.getConnection().prepareStatement(sql)) {
            ...
            ps.setString(6, m.getEtat()); // ✅ ajouté
            ps.executeUpdate();
    ...
    public void modifier(Membre m) {
        // ✅ etat ajouté, index corrigé
        String sql = "UPDATE membre SET nom=?, prenom=?, telephone=?, email=?, " +
                "date_inscription=?, etat=? WHERE id=?";
        ...
        try (PreparedStatement ps = Database.getConnection().prepareStatement(sql)) {
            ...
            ps.setDate(5, Date.valueOf(m.getDateInscription()));
            ps.setString(6, m.getEtat()); // ✅ ajouté
            ps.setInt(7, m.getId());      // ✅ corrigé : était 7 sans etat avant
            ps.executeUpdate();
        } catch (SQLException e) {
